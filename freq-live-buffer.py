...
BUFFER_CHECK_INTERVAL = 0.1  # Seconds to wait before checking buffer
...
def send_gcode_with_glitches(serial_port, baud_rate):
    ...
    try:
        ...
        for command in ["G21", "G90", "M82", "G28", "M104 S200", "M109 S200", "G92 E0"]:
            ser.write(f"{command}\n".encode())
        ...
        for layer_idx in range(NUM_LAYERS):
            ...
            command = f"G1 Z{z_height:.2f} F{MOVE_SPEED}"
            ser.write(f"{command}\n".encode())  # Move to layer height
            print(f"Sending: {command}")
            wait_for_printer_ready(ser)
            ...
            for angle_idx in range(360):
                ...
                command = f"G1 X{x_pos:.2f} Y{y_pos:.2f} F{MOVE_SPEED} E{angle_idx * 0.01:.3f}"
                ser.write(f"{command}\n".encode())
                print(
                    f"Sending: {command} (Note: {note[0] if note else 'None'}, Loudness: {rms:.2f})"
                )
                wait_for_printer_ready(ser)

        # Printer shutdown commands
        for command in ["G28", "M104 S0", "M140 S0", "M84"]:
            ser.write(f"{command}\n".encode())
            print(f"Sending: {command}")
            wait_for_printer_ready(ser)
